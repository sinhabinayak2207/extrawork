...
export const CategoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateCategoryImage = async (categoryId: string, imageUrl: string): Promise<void> => {
    try {
      console.log(`Updating image for category ${categoryId} to ${imageUrl}`);
      ...
      // In a real app, you would update Firestore here using the updateCategoryImage function
      try {
  ...
  const updateCategory = async (categoryId: string, updates: Partial<Category>): Promise<void> => {
    try {
      ...
      // Ensure both image and imageUrl are updated if either is provided
      if (updates.image && !updates.imageUrl) {
        processedUpdates.imageUrl = updates.image;
      } else if (updates.imageUrl && !updates.image) {
        processedUpdates.image = updates.imageUrl;
      }
      ...
      setTimeout(() => {
        ...
        const event = new CustomEvent('categoryUpdated', {
          detail: { categoryId, updates: processedUpdates },
          bubbles: true
        });
        window.dispatchEvent(event);
        document.dispatchEvent(event);
        
        // Cache the updated category in localStorage
        try {
          if (typeof window !== 'undefined' && (processedUpdates.image || processedUpdates.imageUrl)) {
            const categoryCache = JSON.parse(localStorage.getItem('categoryCache') || '{}');
            categoryCache[categoryId] = {
              image: processedUpdates.image || processedUpdates.imageUrl,
              timestamp: new Date().getTime()
            };
            localStorage.setItem('categoryCache', JSON.stringify(categoryCache));
            console.log(`Cached category ${categoryId} in localStorage`);
          }
        } catch (storageError) {
